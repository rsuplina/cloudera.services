...
def build_service_from_params(
    ...
    service_kwargs = {
        ...
        "type": _get_value("type"),
        "display_name": _get_value("display_name"),
        "description": _get_value("description"),
        "tag_service": _get_value("tag_service"),
        "is_enabled": _get_value("is_enabled"),
        "configs": _get_value("configs"),
    }

    # Preserve read-only fields from the existing service if provided.
    if existing is not None:
        service_kwargs.update(
            {
                ...
                "policy_update_time": existing.policy_update_time,
                "tag_version": existing.tag_version,
                "tag_update_time": existing.tag_update_time,
            },
        )

    return RangerService(**service_kwargs)
