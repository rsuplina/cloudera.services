...
def purge_table_by_name(
    ...
    for table in tables:
        try:
            t = table_client.describe_table(
                table[0],
                table[1],
            )  # Get the table to retrieve its ID for deletion
            table_client.delete_table(table[0], t.id)
...
def test_ssb_table_module_present_idempotent_int(
    ...
    assert result["table"]["table_name"] == existing_table.table_name
    if isinstance(existing_table.id, int):
        assert result["table"]["id"] == existing_table.id
...
def test_ssb_table_module_by_id_int(
    ...
    if not isinstance(existing_table.id, int):
...
def test_ssb_table_module_check_mode_create_int(
    ...
    tables = ssb_rest_client.table.list_tables(existing_project.id)
...
def test_ssb_table_module_check_mode_delete_int(
    ...
    tables = ssb_rest_client.table.list_tables(existing_project.id)
    assert any(t.table_name == existing_table.table_name for t in tables)
...
def test_ssb_table_module_diff_mode_delete_int(
    ...
    assert result["diff"]["before"]["table_name"] == existing_table.table_name
...
def test_ssb_table_module_update_with_update_enabled_int(
    ...
    original_table_name = existing_table.table_name
    ...
    if isinstance(existing_table.id, int) and isinstance(updated_table.id, int):
        assert updated_table.id != existing_table.id
...
def test_ssb_table_module_update_with_update_disabled_int(
    ...
    tables = ssb_rest_client.table.list_tables(existing_project.id)
    current_table = next(
        (t for t in tables if t.table_name == existing_table.table_name),
        None,
    )
    ...
    module_args(
        {
            ...
            "project_id": existing_project.id,
            "table_name": existing_table.table_name,
            "type": existing_table.type,
            "metadata": existing_table.metadata,
            "update_enabled": True,
            "state": "present",
        },
    )
