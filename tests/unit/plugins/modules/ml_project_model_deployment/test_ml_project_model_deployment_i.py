...
@unittest.skipUnless(
    os.getenv("CML_ENDPOINT") and os.getenv("CML_API_KEY"),
...
class TestMLRuntimesIntegration(ModuleTestCase):
    ...
    def test_update(self):
        ...
        pprint.pp(e1.value.deployment)
        assert e1.value.deployment

        setup_module_args(
            {
                "project_name": PROJECT_NAME,
                "model_name": MODEL_NAME,
                "id": e1.value.deployment["id"],
                "cpu": 4,
            },
        )
        ...
        pprint.pp(e2.value.deployment)
        assert e2.value.deployment
    ...
    def test_delete(self):
        ...
        pprint.pp(e2.value.deployment)
        assert not e2.value.deployment
