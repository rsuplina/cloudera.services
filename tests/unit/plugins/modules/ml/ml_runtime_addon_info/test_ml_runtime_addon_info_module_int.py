...
def test_list_all(ml_module_args):
    """List all runtime addons in the workspace."""
    ml_module_args()
    ...
    assert isinstance(result["runtime_addons"], list)


def test_filter_no_match(ml_module_args):
    """A non-matching identifier filter returns no addons."""
    ml_module_args({"identifier": "does-not-exist-12345"})

    with pytest.raises(AnsibleExitJson) as e:
        ml_runtime_addon_info.main()

    assert e.value["runtime_addons"] == []
