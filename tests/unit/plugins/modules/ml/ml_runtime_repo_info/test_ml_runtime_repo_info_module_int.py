...
def test_list_all(ml_module_args):
    """List all runtime repositories in the workspace."""
    ml_module_args()
    ...
    assert isinstance(result["runtime_repos"], list)


def test_filter_no_match(ml_module_args):
    """A non-matching name filter returns no repos."""
    ml_module_args({"name": "nonexistent-repo-name-12345"})
